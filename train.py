...
from utils.loss_utils import l1_loss, l2_loss, ssim, Channel_wise_depth_consistency, GrayWorldAssumptionLoss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        depths, t_d, t_b, attn, bs= render_pkg["depths"], render_pkg["transmission_map_d"], render_pkg["backscattering"], render_pkg["attn"],render_pkg["bs"]
        ...
        if iteration <= opt.densify_from_iter:
            ...
            loss = torch.mean((1.0 - opt.lambda_dssim) * Ll1) + opt.lambda_dssim * (1.0 - ssim(mask * image, mask * gt_image)) + l_d + l_g + 0.1 * Ll1_depth
