...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, depth_folder, args):
    cam_infos = []
    cam_centers = []
    ...
    camera_center_mean = np.mean(cam_centers, axis=0)
    sys.stdout.write('\n')
    return cam_infos, camera_center_mean
...
def readColmapSceneInfo(path, images, depth, eval, args, llffhold=8):
    try:
    ...
    depth_dir = "depthmap" if depth == None else depth
    cam_infos_unsorted, camera_center_mean = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir), depth_folder=os.path.join(path, depth_dir), args=args)
